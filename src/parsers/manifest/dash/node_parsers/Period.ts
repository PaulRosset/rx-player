...
export function getPeriodsFromIntermediate(
  ...
  if (period.attributes.start != null) {
    ...
    } else if (prevPeriodInfos || i !== 0) {
      const prevPeriod = prevPeriodInfos || parsedPeriods[i - 1];
    ...
    if (period.attributes.duration != null) {
      ...
      const nextPeriod =  nextPeriodInfos || parsedPeriods[i + 1];
      if (nextPeriod && nextPeriod.start != null && periodStart != null) {
    ...
    const { adaptations } = period.children.adaptations
      ...
      const parsedPeriod : IParsedPeriod = {
        ...
        adaptations,
        linkRef: period.attributes.linkRef,
        linkActuateMode: period.attributes.linkActuateMode as "onRequest"|"onLoad",
      };
  ...
  for (let i = 0; i < periodElement.attributes.length; i++) {
    ...
    switch (attribute.name) {
      ...
      case "bitstreamSwitching":
        ...
        break;
      case "xlink:href":
        res.linkRef = attribute.value;
        break;
      case "xlink:actuate":
        res.linkActuateMode = attribute.value;
        break;
    }
  }
  return res;
